...
function submit(){
    ...
    data.sfrk = $(".sfrk").is(":checked")?1:0;
    $(".item").each(function(){
            data[$(this).attr('name')] = $(this).val()
        })
    data.jztp1=imgArr
    zhpost(JCKSAVE+";JSESSIONID=" + sessionid, data).then( function(result) {
                 console.log(result)   
                 if(result=="success"){
                        if(pppxmlb=="yxk"){
                            window.location.href="../yxkydList.html"
                        }else if(pppxmlb=="zxk"){
                            window.location.href="../zxkydList.html"
                        }else{
                            window.location.href="../jckydList.html"
                        }
                 }else{
                    alert("保存失败")
                 }
...
function imgDelect(obj){
    ...
    hui.confirm('您确定要删除吗？', ['取消','确定'], function(){
        zhpost(DELECTPIC+";JSESSIONID=" + sessionid, data).then( function(result) {
             if(result=="删除成功"){
                $(obj).remove();
                hui.upToast('删除成功!')
             }else{
