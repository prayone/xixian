...
var ECharts = {
    ...
    ChartDataFormate: {
        ...
        FormateGroupData: function (data, type, is_stack,is_top) {//data的格式如上的Result2，type为要渲染的图表类型：可以为line，bar，is_stack表示为是否是堆积图，这种格式的数据多用于展示多条折线图、分组的柱图
            ...
            for (var i = 0; i < group.length; i++) {
                ...
                for (var m = 0; m < xAxis.length; m++) {
                    ...
                    if (!bo) {
                        if (type == "map")
                            temp.push({ name: xAxis[m].name, value: 0 });
                        else
                            temp.push(0);
                    }
                ...
                switch (type) {
                    ...
                    case 'bar':
                        ...
                        var series_temp = { name: group[i], data: temp, type: chart_type,itemStyle: { normal: { label: { show: true, position: 'top' } } } };
                        if (is_top)
                            series_temp = { name: group[i], data: temp, type: chart_type,itemStyle: { normal: { label: { show: false, position: 'top' } } } };
                        if (is_stack)
        ...
        ChartOptionTemplates: {
            ...
            Bars: function (data, name, is_stack,is_top) {//data:数据格式：{name：xxx,group:xxx,value:xxx}...
              
                var bars_dates = ECharts.ChartDataFormate.FormateGroupData(data, 'bar', is_stack,is_top);
